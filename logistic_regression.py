...
class LogisticRegressionExperiment():
    def __init__(self):
        self.clf = LogisticRegressionCV(random_state=0, cv=2, class_weight='balanced')
...
def run_baseline(dataset_name):
    ...
    logistic_regression = LogisticRegressionExperiment()
    logistic_regression.train(x_train, y_train)
    logistic_regression.score(x_test, y_test, (x_train, y_train))
